...
const ChatManagement = () => {
  ...
  useEffect(() => {
    setRows(dashboardData.chats.map((user) => ({ ...user, id: user._id, avatar: user.avatar.map((i) => transformImage(i, 50)), members: user.members.map((i) => transformImage(i.avatar, 50)), creator: {name: user.creator.name, avatar: transformImage(user.creator.avatar, 50)} })));
  }, [])

  return (
    <AdminLayout>
      <Table rows={rows} cols={columns} heading={'All Chats'} rowHeight={'10'} />
    </AdminLayout >
  )
}
